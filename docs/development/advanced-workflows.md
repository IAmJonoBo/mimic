  ...
  private generateReport() {
    ...
    return {
      summary: {
        ...
        duration: this.measurements.reduce((acc, entry) => acc + (entry.duration || 0), 0)
      },
      measurements: byType,
...
export class MemoryMonitor {
  ...
  private logCurrentUsage(current: NodeJS.MemoryUsage) {
    ...
    console.log(`Memory: ${formatted.heapUsed} (${heapDiffMB >= 0 ? '+' : ''}${heapDiffMB} MB)`);
  }
...
export class ContributorOnboarding {
  ...
  private async setupDevelopmentEnvironment() {
    ...
    const steps = [
      ...
      { name: 'Building shared utilities', command: 'npx nx build shared-utils' },
    ...
    const tours = {
      'Design Tokens': async () => {
        ...
        if (runDemo.run) {
          ...
          console.log(chalk.green('  ✅ Check packages/design-tokens/dist/ for output'));
        }
      },
      ...
      'Component Development': async () => {
        ...
        if (openStorybook.open) {
          console.log(chalk.blue('Opening Storybook... (this may take a moment)'));
  ...
  private async createContributorProfile(info: ContributorInfo) {
    ...
    console.log(chalk.gray('  Saved to .contributor-profile.json (git-ignored)'));
  }
}
