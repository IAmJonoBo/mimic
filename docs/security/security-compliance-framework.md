...
export class SupplyChainMonitor {
  async generateSecurityReport(): Promise<SupplyChainReport> {
    ...
    const report: SupplyChainReport = {
      ...
      licenseIssues,
      outdatedPackages,
      recommendations: this.generateRecommendations(vulnerabilities, licenseIssues, outdatedPackages)
    };
      ...
      if (auditData.vulnerabilities) {
        Object.entries(auditData.vulnerabilities).forEach(([name, vuln]: [string, any]) => {
          vulnerabilities.push({
            ...
            cve: vuln.via[0]?.url || '',
            description: vuln.via[0]?.title || vuln.via[0] || 'No description',
  ...
  private async checkLicenses(): Promise<any[]> {
    try {
      ...
      const problematicLicenses = ['GPL', 'AGPL', 'LGPL', 'SSPL', 'OSL', 'EPL', 'MPL'];
      const issues = [];
  ...
  private generateRecommendations(
    ...
    if (highVulns.length > 0) {
      recommendations.push(`⚠️ Fix ${highVulns.length} high-severity vulnerabilities within 7 days`);
    }
    
    // License recommendations
    const highRiskLicenses = licenseIssues.filter(issue => issue.risk === 'high');
    if (highRiskLicenses.length > 0) {
      recommendations.push(`📜 Review ${highRiskLicenses.length} packages with copyleft licenses`);
    }
    
    // Update recommendations
    const majorUpdates = outdatedPackages.filter(pkg => pkg.majorUpdate);
    if (majorUpdates.length > 0) {
      recommendations.push(`📦 Plan migration for ${majorUpdates.length} packages with major updates available`);
    }
    
    // General recommendations
    if (vulnerabilities.length === 0 && licenseIssues.length === 0) {
      recommendations.push('✅ Supply chain security looks good! Continue regular monitoring.');
    }
...
export class VulnerabilityManager {
  ...
  async createResponsePlan(vulnerabilities: any[]): Promise<VulnerabilityResponse[]> {
  ...
  private async planResponse(vulnerability: any): Promise<VulnerabilityResponse> {
  ...
  private async findAlternatives(packageName: string): Promise<string[]> {
    try {
      ...
      const searchOutput = execSync(`npm search ${packageName} --json`, { encoding: 'utf-8' });
