...
class RuntimeGuards {
  ...
  private initializeGuards(): void {
    ...
    if (this.config.logLevel === 'info') {
      // Development mode logging - using console.log is acceptable here
      // eslint-disable-next-line no-console
      console.log('[RuntimeGuards] Collision detection initialized');
    }
  }
    ...
    try {
      ...
      if (collisions.length > 0) {
        this.reportCollision({
          ...
          detected: collisions,
          recommendations: ['Ensure all design tokens use --ds-* prefix', 'Update CSS variables to follow collision-safe naming', 'Check for third-party CSS conflicts'],
        });
      }
    } catch (error) {
  ...
  private detectJSGlobalCollisions(): void {
    ...
    if (potentialConflicts.length > 0) {
      this.reportCollision({
        ...
        recommendations: ['Ensure only authorized design token globals use ds* prefix', 'Check for third-party library conflicts', 'Consider using module imports instead of globals'],
      });
    }
  }
  ...
  private detectNamespaceViolations(): void {
    ...
    if (violations.length > 0) {
      this.reportCollision({
        ...
        detected: violations,
        recommendations: ['Update imports to use libs/tokens/ paths', 'Ensure all token references use ds- prefix', 'Review migration guide for breaking changes'],
      });
    }
  }
