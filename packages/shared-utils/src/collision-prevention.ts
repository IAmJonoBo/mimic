  ...
  try {
    ...
    lines.forEach((line, index) => {
      ...
      if (customPropertyMatch) {
        violations.push(`Line ${index + 1}: "${customPropertyMatch[1]}" should use --ds- prefix (Specify warning prevention)`);
      }

      // Check for class names that might conflict with Tailwind
      const classMatch = line.match(/\.((?!ds-)[a-z-]+)\s*{/);
      if (classMatch) {
        violations.push(`Line ${index + 1}: Class "${classMatch[1]}" should use ds- prefix to prevent Tailwind conflicts`);
      }
    });
  } catch (error) {
    ...
    try {
      ...
      if (portMatch) {
        ...
        if (requiredPorts[configName as keyof typeof requiredPorts] !== port) {
          violations.push(`${configName}: Port ${port} doesn't match Supernova requirement (should be ${requiredPorts[configName as keyof typeof requiredPorts]})`);
        }

        // Check for port conflicts
        if (usedPorts.has(port)) {
          violations.push(`${configName}: Port ${port} conflicts with another Storybook instance (Supernova violation)`);
        }
        usedPorts.add(port);
      } else {
        ...
        urlMatches?.forEach((urlMatch) => {
          ...
          if (portMatch) {
            ...
            if (!validRefPorts.includes(refPort)) {
              violations.push(`${configName}: Composition reference uses invalid port ${refPort} (should be 6006, 7007, or 6008)`);
            }
          }
        });
      }
    ...
    packageJsonPaths.forEach((packagePath) => {
      try {
        ...
        if (packageJson.name && !packageJson.name.startsWith('@mimic/')) {
          violations.push(`${packagePath}: Package name "${packageJson.name}" should use @mimic/ scope (Locofy FAQ compliance)`);
        }
      } catch (error) {
 ...
 */
export function validateCollisionPrevention(options: { cssTokenPath: string; storybookConfigPaths: string[]; metroConfigPath: string; packageJsonPaths: string[] }): CollisionReport {
  ...
  const isValid = tokenNameClashes.length === 0 && storybookPortConflicts.length === 0 && metroDuplicationRisks.length === 0;
...
export function runCollisionCheck(): void {
  ...
  const report = validateCollisionPrevention({
    ...
    storybookConfigPaths: [resolve(workspaceRoot, 'packages/design-system/.storybook/main.ts'), resolve(workspaceRoot, 'packages/design-system/.storybook/main.mobile.ts'), resolve(workspaceRoot, 'packages/design-system/.storybook/main.desktop.ts')],
    metroConfigPath: resolve(workspaceRoot, 'metro.config.js'),
    packageJsonPaths: [resolve(workspaceRoot, 'packages/design-tokens/package.json'), resolve(workspaceRoot, 'packages/shared-utils/package.json'), resolve(workspaceRoot, 'packages/design-system/package.json')],
  });
